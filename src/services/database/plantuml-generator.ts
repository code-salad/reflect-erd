...
function getFullTableName(table: TableSchema): string {
  return table.schema !== 'public'
    ? `${table.schema}.${table.name}`
    : table.name;
}

function generatePrimaryKeyColumns(table: TableSchema): string {
  let result = '';
  if (!table.primaryKey) {
    return result;
  }
  ...
  for (const pkCol of table.primaryKey.columns) {
    ...
    if (column) {
      result += `  * **${column.name}** : ${mapDataType(column.dataType)} <<PK>>`;
      if (column.comment) {
...
function generateRegularColumns(table: TableSchema): string {
  let result = '';
  for (const column of table.columns) {
    ...
    result += `${nullable}${column.name} : ${mapDataType(column.dataType)}`;
...
function generateTableEntity(table: TableSchema): string {
...
function getColumnNotes(column: ColumnSchema, table: TableSchema): string[] {
  const notes: string[] = [];
...
function generateRelationship(table: TableSchema, fk: ForeignKey): string {
  ...
  const relationship = `${escapeName(targetFullName)} ${relationshipType} ${escapeName(sourceFullName)}`;

  return `${relationship} : ${fk.name.replace(/_/g, ' ')}\n`;
}

function generateSimplifiedEntity(table: TableSchema): string {
  let result = '';
  const fullTableName =
    table.schema !== 'public' ? `${table.schema}.${table.name}` : table.name;
  result += `entity ${escapeName(fullTableName)} <<${table.schema}>> {\n`;
...
function generateFullDiagram(schema: TableSchema[]): string {
  let result = '@startuml\n';
...
function generateSimplifiedDiagram(schema: TableSchema[]): string {
  let result = '@startuml\n';
    ...
    for (const fk of table.foreignKeys) {
      ...
      const targetFullName =
        fk.referencedSchema !== 'public'
          ...
          : fk.referencedTable;
      result += `${escapeName(targetFullName)} ||--o{ ${escapeName(sourceFullName)}\n`;
    }
  }
...
export function generatePlantumlSchema(schema: TableSchema[]): {
  full: string;
  simplified: string;
} {
  return {
